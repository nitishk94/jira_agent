...
class _PushFailsContainer(_FakeContainer):
    """A validated fix, but `git push` fails (e.g. no permission) -- must
    not proceed to open_pr against a branch that was never actually pushed."""

    def exec(self, command: str, workdir: str | None = None) -> ExecResult:
        if command.startswith("git push"):
            return ExecResult(exit_code=1, output="remote: Permission to org/repo.git denied")
        return super().exec(command, workdir)


async def test_bug_with_passing_fix_but_failed_push_does_not_open_pr(
    tmp_path: Path, monkeypatch
) -> None:
    triage_result = TriageResult(is_bug=True, issue_type_matched=True, repro_clear=True, reasoning="Clear repro.")
    monkeypatch.setattr(orchestrator_module, "run_triage", _make_triage_fake(triage_result))
    monkeypatch.setattr(
        orchestrator_module, "DockerTicketContainer", lambda mirror_path, branch, image_tag: _PushFailsContainer()
    )
    ...
    assert github_client.prs == []  # never called open_pr
    assert "ENG-1" not in jira_client.statuses  # never transitioned to In Review
    comment = jira_client.comments["ENG-1"][0]
    assert "could not be delivered" in comment
    assert "Permission to org/repo.git denied" in comment
